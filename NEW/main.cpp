...
{
    // Let ImGui process the message first. Only consume the message if ImGui
    // actually wants to capture mouse/keyboard input (prevents swallowing).
    if (ImGui_ImplWin32_WndProcHandler(hWnd, msg, wParam, lParam))
    {
        ImGuiIO& io = ImGui::GetIO();
        if (io.WantCaptureMouse || io.WantCaptureKeyboard)
            return 0; // consumed by ImGui
        // otherwise fall through so the message still reaches the game/OS
    }
...
static HWND CreateOverlayWindow(HINSTANCE hInstance)
{
    ...
    // Make window fully transparent to mouse (click-through) by default.
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
    ...
    // Ensure overlay input mode matches initial visibility of the config UI
